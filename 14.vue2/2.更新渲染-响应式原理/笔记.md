...
1. 使用层面区别：
   ...
   1.2 computed的属性可以是一个函数，或者是一个包含了get、set的对象。当computed函数通过vm调用或函数内部的属性值发生变化时，computed函数会被触发，同时将返回值重新渲染到页面。
2. 实现原理区别：
...
3. 具体实现原理：(面试官没问可忽略)
   ...
   3.2 computed是在initComputed时，通过vm.defineProperty完成对computed的函数完成代理，通过vm调用函数时即会触发函数的getter。同时会创建computed watcher，将函数名和computed watcher进行映射存储到vm._computedWatchers上。
      3.2.1 在Dep中创建一个wathcer的栈存储render watcher和computed watcher，当函数的getter被触发的时候，利用栈会分别完成data属性对render watcher和computed watcher的收集。收集完后再依次清空栈，清空render watcher是利用dep和watcher多对多的关系，通过
      3.2.1 利用Dep中的栈存储render watcher和computed watcher，栈的最新值和Dep.target保持一致。computed函数被vm调用时，将对应的computed watcher，重置Dep.target上。computed函数内部的data属性即可收集到computed watcher，然后把栈进行pop，   Dep.target改为render watcher。dep和watcher是多对多的关系，通过computed watcher找到所有的dep，让dep再收集一遍render watcher。
