...
# watch和computed的区别
1. 使用层面区别：
   1. watch是对data的属性值直接监测，属性值发生变化时，watch对应的函数会被执行。watch可以设置deep属性，对对象类型的数值进行深度观察。
   2. computed是对data属性的二次封装，是可能对多个属性值同时监测。当computed函数通过vm调用或函数内部的属性值发生变化时，computed函数会被触发，同时将返回值重新渲染到页面。
2. 实现原理区别：
   1. watch是在组件初始化的时候，生命周期created之前完成的依赖收集。在initWatch时，会创建user watcher，同时触发watcher的get方法，根据watch对应的data属性，触发data属性的getter完成对user watcher的依赖收集。
   2. computed是在initComputed时，通过vm.defineProperty完成对computed函数名的观察，调用函数时即会触发函数的getter。同时会创建computed watcher，将函数名和computed watcher进行映射存储到vm._computedWatchers上。
      2.1 在Dep中创建一个wathcer的队列存储render watcher和computed watcher，当函数的getter被触发的时候，利用队列会分别完成data属性对render watcher和cmputed watcher的收集。收集完后再依次清空队列。
      2.2 当computed函数内部的data属性发生变化时，即可触发属性的computed watcher和render wathcer。此时computed wathcer只需要将dirty改为true，render wathcer执行update时，会带动computed函数的getter完成最新数据的渲染。
      2.3 当computed函数运行前，会时如果dirty为true代表本轮还未进行过计算，然后通过watcher.evaluate完成计算后，把dirty改为false。这样重复调用computed函数时，computed函数只会被执行一次。